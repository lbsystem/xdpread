...
func createsocket(ifidx int, queueid int) *xdp.Socket {
...
func NewAFXDP(ifname string, port, queueID int) (*xdp.Socket, *udpObjects, link.Link) {

	bpf, l, link := newEbpf(ifname, uint16(port))
	
	xsk := createsocket(link.Index, queueID)
...
func DecodeUdp(b []byte) (src, dst *net.UDPAddr, data []byte, e error) {
	...
	if len(b) < 42 {
		return nil, nil, nil, fmt.Errorf("It is not an IP data")
	}
